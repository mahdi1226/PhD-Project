...
//
// MMS MODE: Returns zero because MMS source terms are derived without h_a.
//           Including h_a would create O(1) modeling error → zero EOC.
// ============================================================================
template <int dim>
    ...
    double current_time)
{
    ...
    // ========================================================================
    // MMS GUARD: Applied field must be disabled during MMS testing
    //
    // The MMS exact solutions φ*, M* were derived WITHOUT dipole fields.
    // Including h_a here would add an O(1) error that doesn't decrease with h,
    // causing convergence rates to collapse to zero.
    // ========================================================================
    if (params.enable_mms)
        return h_a;

    // Uniform field mode: return constant field (with optional ramp)
    if (params.dipoles.use_uniform_field)
    {
        const double ramp_factor = (params.dipoles.ramp_time > 0.0)
            ? std::min(current_time / params.dipoles.ramp_time, 1.0)
            : 1.0;
        for (unsigned int d = 0; d < dim; ++d)
            h_a[d] = params.dipoles.uniform_field_value[d] * ramp_factor;
        return h_a;
    }
    ...
    else
    {
        ...
        const double delta = params.dipoles.regularization * min_dipole_dist;
