...
//
// RHS:
//   f_M:   (1/dt)(M^{k-1}, Z) + (1/tau_M) chi(theta) (h_a, Z)  [time + relaxation h_a part]
         ...
         ++cell_mag, ++cell_U, ++cell_theta)
    {
        ...
        for (unsigned int q = 0; q < n_q_cell; ++q)
        {
            ...
            for (unsigned int i = 0; i < dofs_per_cell; ++i)
            {
                ...
                // M block: +(1/tau_M) chi(theta) (h_a, Z_i) — relaxation source (h_a part)
                // Paper Eq 42c: target is χ(θ)(∇φ + h_a). The ∇φ part is on LHS
                // (C_M_phi coupling), the known h_a part goes here on the RHS.
