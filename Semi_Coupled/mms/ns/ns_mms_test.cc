...
void NSMMSConvergenceResult::compute_rates()
{
    ...
    for (size_t i = 1; i < n; ++i)
    {
        ...
        if (results[i - 1].ux_Linf > 1e-15 && results[i].ux_Linf > 1e-15)
            ux_Linf_rate[i] = std::log(results[i - 1].ux_Linf / results[i].ux_Linf) / log_h;
        if (results[i - 1].uy_Linf > 1e-15 && results[i].uy_Linf > 1e-15)
            uy_Linf_rate[i] = std::log(results[i - 1].uy_Linf / results[i].uy_Linf) / log_h;
        if (results[i - 1].p_Linf > 1e-15 && results[i].p_Linf > 1e-15)
            p_Linf_rate[i] = std::log(results[i - 1].p_Linf / results[i].p_Linf) / log_h;
    }
}
...
void NSMMSConvergenceResult::print() const
{
    ...
    std::cout << std::left
              ...
              << std::setw(12) << "div_U"
              << std::setw(12) << "ux_Linf" << std::setw(8) << "rate"
              << std::setw(12) << "uy_Linf" << std::setw(8) << "rate"
              << std::setw(12) << "p_Linf" << std::setw(8) << "rate"
              << "\n";
    ...
    for (size_t i = 0; i < results.size(); ++i)
    {
        std::cout << std::left << std::setw(6) << results[i].refinement
                  ...
                  << std::setw(12) << results[i].ux_Linf
                  << std::fixed << std::setprecision(2)
                  << std::setw(8) << ux_Linf_rate[i]
                  << std::scientific << std::setprecision(2)
                  << std::setw(12) << results[i].uy_Linf
                  << std::fixed << std::setprecision(2)
                  << std::setw(8) << uy_Linf_rate[i]
                  << std::scientific << std::setprecision(2)
                  << std::setw(12) << results[i].p_Linf
                  << std::fixed << std::setprecision(2)
                  << std::setw(8) << p_Linf_rate[i]
                  << "\n";
    }
    std::cout << "========================================\n";
...
void NSMMSConvergenceResult::write_csv(const std::string& filename) const
{
    ...
    file << "refinement,h,n_dofs,ux_L2,ux_L2_rate,ux_H1,ux_H1_rate,"
         << "uy_L2,uy_L2_rate,p_L2,p_L2_rate,div_U,"
         << "ux_Linf,ux_Linf_rate,uy_Linf,uy_Linf_rate,p_Linf,p_Linf_rate,time\n";
    ...
    for (size_t i = 0; i < results.size(); ++i)
    {
        file << results[i].refinement << ","
             ...
             << results[i].ux_Linf << ","
             << std::fixed << std::setprecision(2) << ux_Linf_rate[i] << ","
             << std::scientific << results[i].uy_Linf << ","
             << std::fixed << uy_Linf_rate[i] << ","
             << std::scientific << results[i].p_Linf << ","
    ...
    MPI_Comm mpi_comm)
{
    ...
    double local_ux_Linf = 0.0, local_uy_Linf = 0.0, local_p_Linf = 0.0;
    ...
    for (; ux_cell != ux_dof_handler.end(); ++ux_cell, ++uy_cell, ++p_cell)
    {
        ...
        for (unsigned int q = 0; q < n_q_points; ++q)
        {
            ...
            local_ux_Linf = std::max(local_ux_Linf, std::abs(ux_err));
            local_uy_Linf = std::max(local_uy_Linf, std::abs(uy_err));
    ...
    double global_ux_Linf = 0.0, global_uy_Linf = 0.0, global_p_Linf = 0.0;
    MPI_Allreduce(&local_ux_Linf, &global_ux_Linf, 1, MPI_DOUBLE, MPI_MAX, mpi_comm);
    MPI_Allreduce(&local_uy_Linf, &global_uy_Linf, 1, MPI_DOUBLE, MPI_MAX, mpi_comm);
    MPI_Allreduce(&local_p_Linf, &global_p_Linf, 1, MPI_DOUBLE, MPI_MAX, mpi_comm);
    ...
    error.ux_Linf = global_ux_Linf;
    error.uy_Linf = global_uy_Linf;
    error.p_Linf = global_p_Linf;
    ...
    MPI_Comm mpi_comm)
{
    ...
    for (unsigned int ref : refinements)
    {
        ...
        res.ux_Linf = errors.ux_Linf;
        res.uy_Linf = errors.uy_Linf;
        res.p_Linf = errors.p_Linf;
