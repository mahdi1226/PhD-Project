...
//
// Tests ALL four subsystems coupled together:
//   1. CH: ∂θ/∂t + U·∇θ = γΔψ + f_θ
//   2. Poisson: -Δφ = -∇·M + f_φ
//   3. Magnetization: ∂M/∂t + M/τ = χH/τ + f_M  (advection disabled for MMS)
//   4. NS: ∂U/∂t + (U·∇)U - νΔU + ∇p = μ₀[(M·∇)H + ½(∇·M)H] + f_NS
//
// Time stepping (matches production PhaseFieldProblem::run()):
//   1. Solve CH ONCE (uses U^{n-1})
//   2. Picard iteration for Poisson ↔ Magnetization
//   3. Solve NS ONCE (uses θ^{n-1}, converged M, H)
//
// Uses PRODUCTION code paths:
//   - assemble_ch_system() with enable_mms=true
//   - solve_ch_system()
//   - assemble_poisson_rhs() with enable_mms=true
    ...
    MPI_Comm mpi_communicator)
{
    ...
    const double t_end = 0.6;  // Extended for long-time stability test (was 0.2)
    ...
    // NS DoFs (Q2 velocity, CG pressure — DG pressure has stability issues in MMS)
    ...
    // Magnetization DoFs (DG-Q1 per paper requirement A2)
    ...
    setup_poisson_constraints_and_sparsity<dim>(
        phi_dof, phi_owned, phi_relevant,
    ...
    // Assemble Poisson matrix once (constant)
    assemble_poisson_matrix<dim>(phi_dof, phi_constraints, phi_matrix);
    ...
    // NS IC
    NSExactVelocityX<dim> ux_exact_ic(t_start, L_y);
    NSExactVelocityY<dim> uy_exact_ic(t_start, L_y);
    {
        ...
        dealii::VectorTools::interpolate(ux_dof, ux_exact_ic, tmp_ux);
        dealii::VectorTools::interpolate(uy_dof, uy_exact_ic, tmp_uy);
        ux_old = tmp_ux;
        uy_old = tmp_uy;
    }
    ...
    // Poisson IC
    PoissonExactSolution<dim> phi_ic(t_start, L_y);
    dealii::VectorTools::interpolate(phi_dof, phi_ic, phi_solution);
    phi_vec = phi_solution;
    ...
    // ========================================================================
    // MMS needs tight Picard convergence: the production defaults (tol=0.05,
    // max=7, omega=0.35) are tuned for physical runs where temporal error
    // dominates. For MMS spatial-convergence tests, the Picard loop must
    // converge to near machine precision so that coupling residual doesn't
    // create an h-independent error floor.
    const double picard_tol = 1e-10;
    const double omega = 1.0;  // No under-relaxation (same as POISSON_MAG test)
    ...
    for (unsigned int step = 0; step < n_time_steps; ++step)
    {
        ...
        // ====================================================================
        // Step 2: Picard iteration for Poisson ↔ Magnetization
        // ====================================================================
        ...
        for (unsigned int picard_iter = 0; picard_iter < max_picard; ++picard_iter)
        {
            ...
            // Solve Magnetization: ∂M/∂t + M/τ = χH/τ + f_M
            M_matrix = 0;
            Mx_rhs = 0;
            My_rhs = 0;
            ...
            mag_assembler->assemble(
                ...
                phi_vec,         // Potential (for H = h_a - ∇φ)
                theta_rel,       // Phase field (for χ(θ))
            ...
            // DIAGNOSTIC: Check residual with exact M interpolation
            // NOTE: All ranks must participate (interpolate/vmult are collective)
            if (picard_iter == 0)
            {
                MagExactMx<dim> Mx_exact_fn(current_time, L_y);
                MagExactMy<dim> My_exact_fn(current_time, L_y);
                dealii::TrilinosWrappers::MPI::Vector Mx_exact_owned(M_owned, mpi_communicator);
                dealii::TrilinosWrappers::MPI::Vector My_exact_owned(M_owned, mpi_communicator);
                dealii::VectorTools::interpolate(M_dof, Mx_exact_fn, Mx_exact_owned);
                dealii::VectorTools::interpolate(M_dof, My_exact_fn, My_exact_owned);

                dealii::TrilinosWrappers::MPI::Vector Ax(M_owned, mpi_communicator);
                dealii::TrilinosWrappers::MPI::Vector Ay(M_owned, mpi_communicator);
                M_matrix.vmult(Ax, Mx_exact_owned);
                M_matrix.vmult(Ay, My_exact_owned);
                Ax -= Mx_rhs;
                Ay -= My_rhs;
                double resid_x = Ax.l2_norm();
                double resid_y = Ay.l2_norm();
                double rhs_norm = Mx_rhs.l2_norm() + My_rhs.l2_norm();
                pcout << "    [DIAG] ||A*M_exact - rhs||: Mx=" << std::scientific << std::setprecision(3)
                      << resid_x << ", My=" << resid_y
                      << ", rel=" << (resid_x + resid_y) / (rhs_norm + 1e-14) << std::endl;
            }
            ...
            if (this_rank == 0)
                pcout << "    [Picard step=" << step << " iter=" << picard_iter
                      << "] residual=" << std::scientific << std::setprecision(3)
                      << picard_residual << std::endl;
        ...
        // ====================================================================
        // Step 3: Solve NS with Kelvin force
        // ====================================================================
        ns_matrix = 0;
        ns_rhs = 0;
    ...
    // NS errors
    {
        ...
        // Pressure (with mean subtraction for DG pressure)
    ...
    // Poisson errors
    {
        ...
        PoissonMMSError phi_err = compute_poisson_mms_errors_parallel<dim>(
            phi_dof, phi_vec, current_time, L_y, mpi_communicator);
    ...
    MPI_Comm mpi_communicator)
{
    ...
    if (this_rank == 0)
    {
        ...
        std::cout << "========================================\n";
        std::cout << "  Tests: CH + NS + Poisson + Magnetization\n";
        std::cout << "  Couplings:\n";
        std::cout << "    - CH advected by NS (U·∇θ)\n";
        std::cout << "    - Poisson ↔ Magnetization (Picard)\n";
