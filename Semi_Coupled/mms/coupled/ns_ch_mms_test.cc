...
//   - solve_ns_system_direct_parallel() (Direct solver - MUMPS/UMFPACK)
//   - assemble_ch_system()
//   - solve_ch_system()
//
// NOTE (2026-01-18): Uses direct solver instead of Schur preconditioner
//   - Schur preconditioner requires contiguous velocity IndexSets
//   - With separate ux/uy DoFHandlers, velocity IndexSets are non-contiguous
//   - Direct solver is correct and fast for MMS test sizes (ref ≤ 4)
//   - Production ferrofluid code also uses direct solver for these sizes
//
    ...
    MPI_Comm mpi_communicator)
{
    ...
    // NOTE: pressure_mass and vel_owned removed - only needed for Schur preconditioner
    // Direct solver doesn't require these
    ...
    for (unsigned int step = 0; step < n_time_steps; ++step)
    {
        ...
        // ====================================================================
        // Step 1: Update CH boundary conditions
        // CRITICAL: Pass L_y to boundary condition functions
        // ====================================================================
        ...
        // ====================================================================
        // Step 2: Solve CH with advection using U^{n-1}
        // Paper's Block-GS ordering: CH is solved FIRST, using velocity from
        // the previous time step. The MMS source CHSourceThetaWithConvection
        // evaluates velocity at t_old to match this ordering.
        // ====================================================================
        ch_matrix = 0;
        ch_rhs = 0;
        assemble_ch_system<dim>(theta_dof, psi_dof, theta_old,
                                ux_dof, uy_dof, // Velocity DoF handlers
                                ux_old, uy_old, // Velocity from previous step (U^{n-1})
        ...
        // ====================================================================
        // Step 3: Solve NS
        // Paper's Block-GS ordering: NS is solved AFTER CH.
        // ====================================================================
        ns_matrix = 0;
        ns_rhs = 0;
        ...
        // Update old solutions AFTER both CH and NS solves
        ux_old = ux_sol;
        uy_old = uy_sol;
    }
