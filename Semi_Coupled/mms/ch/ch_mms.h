...
//
// Assembler weak form (ch_assembler.cc):
//
//   (1/dt)(θ, Λ) + γ(∇ψ, ∇Λ) = (1/dt)(θ_old, Λ) + (S_θ, Λ)
//
// After IBP (with Λ=0 on ∂Ω): +γ(∇ψ,∇Λ) → -γΔψ in strong form.
//
// Strong form: (θ - θ_old)/dt - γΔψ = S_θ
//
...
//
// After IBP (with Υ=0 on ∂Ω): -ε(∇θ,∇Υ) → +εΔθ in strong form.
//
// Strong form: ψ + εΔθ - (1/η)θ = (1/ε)f_old - (1/η)θ_old + S_ψ
//
//   S_ψ = ψ*^n + ε Δθ*^n - (1/ε) f(θ*^{n-1}) - (1/η)(θ*^n - θ*^{n-1})
//
// NOTE: uses lagged nonlinearity f_old = θ_old^3 - θ_old, and η = ε.
// ============================================================================
template <int dim>
...
//
// Assembler weak form (ch_assembler.cc, Nochetto Eq. 42a):
//
//   (1/dt)(θ, Λ) + γ(∇ψ, ∇Λ) = (1/dt)(θ_old, Λ) + (U θ_old, ∇Λ) + (S_θ, Λ)
//
// After IBP: +γ(∇ψ,∇Λ) → -γΔψ, and (Uθ_old,∇Λ) → -(U·∇θ_old, Λ)
//
// Strong form: (θ-θ_old)/dt - γΔψ + U·∇θ_old = S_θ
//
//   S_θ = (θ*^n - θ*^{n-1})/dt + U*^n · ∇θ*^{n-1} - γ Δψ*^n
//
// Paper's Block-GS ordering: CH is solved FIRST, so it uses velocity U from
// the PREVIOUS time step (t_old). The gradient ∇θ is also at t_old (lagged).
// This matches Nochetto et al. Eq. 42a: U^{n-1} · ∇θ^{n-1}.
//
