...
public:
    ...
    void print_header()
    {
        ...
        std::string timestamp = get_timestamp_local();  // No MPI — rank-0 only context
                     ...
                     unsigned int prev_dofs = 0)
    {
        ...
        // Block-GS note (only if > 1 iteration was needed)
        if (data.bgs_iterations > 1 && data.step % 10 == 0)
        {
            std::cout << "[BGS] Step " << data.step
                << ": " << data.bgs_iterations << " iters, residual = "
                << std::scientific << std::setprecision(2) << data.bgs_residual << "\n";
        }

        // Interface change note (only if significant)
        double delta_y = data.interface_y_max - initial_interface_y_;
        if (std::abs(delta_y) > 0.005 && data.step % 10 == 0)
        {
            std::cout << "[+] Step " << data.step
