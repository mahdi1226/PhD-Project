...
{
    // DG flux sparsity: use DynamicSparsityPattern for robustness with AMR
    // (AMR subfaces create variable coupling patterns that TrilinosWrappers::SparsityPattern
    //  may not pre-allocate correctly)
    dealii::DynamicSparsityPattern dsp(M_locally_relevant);
    dealii::DoFTools::make_flux_sparsity_pattern(M_dof_handler, dsp);
    dealii::SparsityTools::distribute_sparsity_pattern(
        dsp, M_dof_handler.locally_owned_dofs(), mpi_communicator, M_locally_relevant);

    M_matrix.reinit(M_locally_owned, M_locally_owned, dsp, mpi_communicator);

    pcout << "[Magnetization Setup] n_dofs = " << M_dof_handler.n_dofs()
          << ", locally_owned = " << M_locally_owned.n_elements()
          << ", nnz = " << M_matrix.n_nonzero_elements() << " (DG flux)\n";
}
