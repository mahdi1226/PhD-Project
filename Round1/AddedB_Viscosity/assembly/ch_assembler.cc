        ...
        for (unsigned int q = 0; q < n_q_points; ++q)
        {
            ...
            for (unsigned int i = 0; i < dofs_per_cell; ++i)
            {
                ...
                for (unsigned int j = 0; j < dofs_per_cell; ++j)
                {
                    ...
                    local_matrix(i_psi, j_theta) -= eps * (grad_theta_j * grad_Upsilon_i) * JxW; // FIX: -eps
                    local_matrix(i_psi, j_theta) -= (1.0 / eta) * theta_j * Upsilon_i * JxW;     // FIX: -1/eta
                }

                // Eq 42a RHS: (θ^{k-1}/τ, Λ) + (u^k θ^{k-1}, ∇Λ)
                ...
                // RHS: +1/ε f - 1/η θ^{k-1}
                local_rhs(i_psi) += (1.0 / eps) * f_old * Upsilon_i * JxW;        // FIX: +1/eps
                local_rhs(i_psi) -= (1.0 / eta) * theta_old_q * Upsilon_i * JxW;  // FIX: -1/eta
            }
        }
