...
// ============================================================================
// Exact magnetic field H* = -∇φ* (convenience function for coupled MMS)
//
// H* = (t·π·sin(πx)·cos(πy/L_y), t·(π/L_y)·cos(πx)·sin(πy/L_y))
// ============================================================================
template <int dim>
dealii::Tensor<1, dim> poisson_mms_exact_H(
    const dealii::Point<dim>& pt,
    double time,
    double L_y = 1.0)
{
    ...
    H[0] = time * M_PI * std::sin(M_PI * x) * std::cos(M_PI * y / L_y);
    if constexpr (dim >= 2)
        H[1] = time * (M_PI / L_y) * std::cos(M_PI * x) * std::sin(M_PI * y / L_y);
    return H;
}
    ...
    MPI_Comm mpi_communicator)
{
    ...
    // Second pass: compute L2 error and L∞ with mean-shifted solution (LOCAL)
    // NOTE: L∞ must also use mean-shifted solution since Poisson with Neumann BC
    // is unique only up to a constant.
    double local_L2_sq = 0.0;
    double local_Linf_shifted = 0.0;
    ...
    for (const auto& cell : phi_dof_handler.active_cell_iterators())
    {
        ...
        for (unsigned int q = 0; q < n_q_points; ++q)
        {
            ...
            local_Linf_shifted = std::max(local_Linf_shifted, std::abs(value_error));
        }
    }

    // Global reductions for L2 and L∞
    double global_L2_sq = 0.0;
    double global_Linf_shifted = 0.0;
    MPI_Allreduce(&local_L2_sq, &global_L2_sq, 1, MPI_DOUBLE, MPI_SUM, mpi_communicator);
    MPI_Allreduce(&local_Linf_shifted, &global_Linf_shifted, 1, MPI_DOUBLE, MPI_MAX, mpi_communicator);

    errors.L2_error = std::sqrt(global_L2_sq);
    errors.H1_error = std::sqrt(global_H1_sq);
    errors.Linf_error = global_Linf_shifted;
