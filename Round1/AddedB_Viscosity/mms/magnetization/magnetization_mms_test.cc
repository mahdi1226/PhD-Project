...
void MagMMSConvergenceResult::compute_rates()
{
    ...
    for (size_t i = 1; i < results.size(); ++i)
    {
        ...
        const double linf_fine = results[i].M_Linf;
        const double linf_coarse = results[i-1].M_Linf;

        if (linf_coarse > 1e-15 && linf_fine > 1e-15)
            M_Linf_rates.push_back(std::log(linf_coarse / linf_fine) / std::log(h_coarse / h_fine));
        else
            M_Linf_rates.push_back(0.0);
    }
}
...
void MagMMSConvergenceResult::print() const
{
    ...
    for (size_t i = 0; i < results.size(); ++i)
    {
        ...
        std::cout << std::left << std::setw(6) << r.refinement
                  ...
                  << std::fixed << std::setprecision(2)
                  << std::setw(8) << (i > 0 && !M_Linf_rates.empty() ? M_Linf_rates[i-1] : 0.0)
                  << std::fixed << std::setprecision(2)
...
void MagMMSConvergenceResult::write_csv(const std::string& filename) const
{
    ...
    if (!file.is_open())
    {
        std::cerr << "[Mag MMS] Failed to open " << filename << " for writing\n";
        return;
    }

    file << "refinement,h,n_dofs,M_L2,M_L2_rate,Mx_L2,My_L2,"
         << "M_Linf,M_Linf_rate,Mx_Linf,My_Linf,total_time\n";
    ...
    for (size_t i = 0; i < results.size(); ++i)
    {
        ...
        file << r.refinement << ","
             ...
             << r.M_Linf << ","
             << std::fixed << std::setprecision(2)
             << (i > 0 && !M_Linf_rates.empty() ? M_Linf_rates[i-1] : 0.0) << ","
             << std::scientific << std::setprecision(6)
             << r.Mx_Linf << ","
             << r.My_Linf << ","
...
{
    // Default overload: 100 time steps
    return run_magnetization_mms_single(refinement, params, solver_type, 100, mpi_communicator);
}
    ...
    MPI_Comm mpi_communicator)
{
    ...
    const unsigned int n_steps = n_time_steps_in;
...
// ============================================================================
// run_magnetization_mms_standalone - Overload with variable time steps
// ============================================================================
MagMMSConvergenceResult run_magnetization_mms_standalone(
    const std::vector<unsigned int>& refinements,
    const Parameters& params,
    ...
    MPI_Comm mpi_communicator)
{
    ...
    result.fe_degree = params.fe.degree_magnetization;
    result.solver_type = solver_type;
    result.expected_L2_rate = params.fe.degree_magnetization + 1;
    ...
    for (unsigned int ref : refinements)
    {
        ...
        MagMMSResult r = run_magnetization_mms_single(
            ref, params, solver_type, n_time_steps, mpi_communicator);
        result.results.push_back(r);
...
// ============================================================================
// TRANSPORT TEST: Magnetization with NS MMS velocity
//
// Isolates DG transport from ALL coupling:
//   - Uses interpolated NS MMS velocity (div-free, zero on ∂Ω)
//   - Sets chi_0 = 0 → no χH coupling (M purely from mass + transport)
//   - Uses production assembler + solver
// ============================================================================
#include "mms/ns/ns_mms.h"

static MagMMSResult run_magnetization_transport_single(
    unsigned int refinement,
    const Parameters& params,
    unsigned int n_time_steps_in,
    MPI_Comm mpi_communicator)
{
    MagMMSResult result;
    result.refinement = refinement;
    result.solver_type = MagSolverType::Direct;
    ...
    Parameters mms_params = params;
    mms_params.enable_mms = true;
    mms_params.physics.chi_0 = 0.0;  // Disable magnetic coupling
    // Use production τ_M = 1e-6 (mass dominates, transport is perturbation)
    ...
    result.n_dofs = 2 * M_dof_handler.n_dofs();
    ...
    // Velocity (non-zero! from NS MMS)
    dealii::TrilinosWrappers::MPI::Vector Ux_owned(U_locally_owned, mpi_communicator);
    dealii::TrilinosWrappers::MPI::Vector Uy_owned(U_locally_owned, mpi_communicator);
    dealii::TrilinosWrappers::MPI::Vector Ux(U_locally_owned, U_locally_relevant, mpi_communicator);
    dealii::TrilinosWrappers::MPI::Vector Uy(U_locally_owned, U_locally_relevant, mpi_communicator);

    // phi=0, theta=1 (dummy, chi=0 so no coupling)
    dealii::TrilinosWrappers::MPI::Vector phi(phi_locally_owned, phi_locally_relevant, mpi_communicator);
    dealii::TrilinosWrappers::MPI::Vector theta(theta_locally_owned, theta_locally_relevant, mpi_communicator);
    phi = 0.0;
    theta = 1.0;
    ...
    // ========================================================================
    // Initialize M with L2 projection at t_start
    // ========================================================================
    MagExactMx<dim> exact_Mx_init(t_start, L_y);
    MagExactMy<dim> exact_My_init(t_start, L_y);
    {
        ...
        for (const auto& cell : M_dof_handler.active_cell_iterators())
        {
            ...
            fe_values.reinit(cell);
            local_mass = 0; local_rhs_x = 0; local_rhs_y = 0;
    ...
    // Initialize velocity: NS MMS velocity (solenoidal, zero on ∂Ω)
    // U·n = 0 on boundary ensures DG transport needs no boundary face terms
    {
        NSExactVelocityX<dim> exact_Ux(t_start, L_y);
        NSExactVelocityY<dim> exact_Uy(t_start, L_y);
        dealii::VectorTools::interpolate(U_dof_handler, exact_Ux, Ux_owned);
        dealii::VectorTools::interpolate(U_dof_handler, exact_Uy, Uy_owned);
        Ux = Ux_owned;
        Uy = Uy_owned;
    }
    ...
    for (unsigned int step = 0; step < n_steps; ++step)
    {
        ...
        // Update velocity to current time (NS MMS is time-dependent)
        {
            NSExactVelocityX<dim> exact_Ux_now(current_time, L_y);
            NSExactVelocityY<dim> exact_Uy_now(current_time, L_y);
            dealii::VectorTools::interpolate(U_dof_handler, exact_Ux_now, Ux_owned);
            dealii::VectorTools::interpolate(U_dof_handler, exact_Uy_now, Uy_owned);
            Ux = Ux_owned;
            Uy = Uy_owned;
        }
    ...
    MPI_Comm mpi_communicator)
{
    ...
    if (this_rank == 0)
    {
        std::cout << "\n[MAG_TRANSPORT] Magnetization + DG Transport MMS Test\n";
        std::cout << "  Velocity: NS MMS (solenoidal, zero on boundary)\n";
        std::cout << "  chi_0 = 0 (no magnetic coupling)\n";
        std::cout << "  Time steps: " << n_time_steps << "\n";
        std::cout << "  FE degree: DG" << params.fe.degree_magnetization << "\n";
        std::cout << "  Expected L2 rate: " << result.expected_L2_rate << "\n\n";
    }
    ...
    for (unsigned int ref : refinements)
    {
        ...
        MagMMSResult r = run_magnetization_transport_single(
