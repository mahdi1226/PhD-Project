...
void CHMMSConvergenceResult::compute_rates()
{
    ...
    for (size_t i = 1; i < results.size(); ++i)
    {
        ...
        if (results[i-1].theta_Linf > 1e-15 && results[i].theta_Linf > 1e-15)
        {
            theta_Linf_rates.push_back(
                std::log(results[i-1].theta_Linf / results[i].theta_Linf) / log_h_ratio);
        }
        else
        {
            theta_Linf_rates.push_back(0.0);
        }

        if (results[i-1].psi_Linf > 1e-15 && results[i].psi_Linf > 1e-15)
        {
            psi_Linf_rates.push_back(
                std::log(results[i-1].psi_Linf / results[i].psi_Linf) / log_h_ratio);
        }
        else
        {
            psi_Linf_rates.push_back(0.0);
        }
    }
}
...
void CHMMSConvergenceResult::print() const
{
    ...
    for (size_t i = 0; i < results.size(); ++i)
    {
        ...
        std::cout << std::left << std::setw(5) << r.refinement
                  ...
                  << std::scientific << std::setprecision(2)
                  << std::setw(10) << r.theta_Linf
                  << std::fixed << std::setprecision(2)
                  << std::setw(6) << (i > 0 && !theta_Linf_rates.empty() ? theta_Linf_rates[i-1] : 0.0)
                  << std::scientific << std::setprecision(2)
                  << std::setw(10) << r.psi_Linf
                  << std::fixed << std::setprecision(2)
                  << std::setw(6) << (i > 0 && !psi_Linf_rates.empty() ? psi_Linf_rates[i-1] : 0.0)
                  << "\n";
    }
}
...
void CHMMSConvergenceResult::write_csv(const std::string& filename) const
{
    ...
    file << "refinement,h,n_dofs,theta_L2,theta_L2_rate,theta_H1,theta_H1_rate,"
         << "psi_L2,psi_L2_rate,"
         << "theta_Linf,theta_Linf_rate,psi_Linf,psi_Linf_rate,total_time\n";
    ...
    for (size_t i = 0; i < results.size(); ++i)
    {
        ...
        file << r.refinement << ","
             ...
             << r.theta_Linf << ","
             << (i > 0 && !theta_Linf_rates.empty() ? theta_Linf_rates[i-1] : 0.0) << ","
             << r.psi_Linf << ","
             << (i > 0 && !psi_Linf_rates.empty() ? psi_Linf_rates[i-1] : 0.0) << ","
    ...
    MPI_Comm mpi_communicator)
{
    ...
    for (const auto& cell : theta_dof_handler.active_cell_iterators())
    {
        ...
        for (unsigned int q = 0; q < n_q_points; ++q)
        {
            ...
            local_theta_Linf = std::max(local_theta_Linf, std::abs(theta_err));
            local_psi_Linf = std::max(local_psi_Linf, std::abs(psi_err));
        }
    }
    ...
    MPI_Comm mpi_communicator)
{
    ...
    for (const auto& r : conv_result.results)
    {
        ...
        mapped.theta_Linf = r.theta_Linf;
        mapped.psi_Linf = r.psi_Linf;
