    ...
    MPI_Comm mpi_communicator)
{
    ...
    dealii::IndexSet dummy_theta_owned = dummy_theta_dof.locally_owned_dofs();
    dealii::IndexSet dummy_theta_relevant = dealii::DoFTools::extract_locally_relevant_dofs(dummy_theta_dof);
    ...
    dealii::TrilinosWrappers::MPI::Vector theta_dummy(dummy_theta_owned, dummy_theta_relevant, mpi_communicator);
    Ux_dummy = 0;
    Uy_dummy = 0;
    theta_dummy = 1.0;
    ...
    for (unsigned int step = 0; step < n_time_steps; ++step)
    {
        ...
        // --------------------------------------------------------------------
        // Steps 1-2: Picard iteration for Poisson-Magnetization coupling
        // --------------------------------------------------------------------
        const unsigned int max_picard = 10;
        const double picard_tol = 1e-8;

        for (unsigned int picard = 0; picard < max_picard; ++picard)
        {
            // TEMPORARY: Use exact φ to verify NS converges when φ is correct
            PoissonExactSolution<dim> phi_exact(current_time, L_y);
            dealii::VectorTools::interpolate(phi_dof, phi_exact, phi_solution);
            phi_ghosted = phi_solution;

            /* COMMENT OUT:
            // Step 1: Solve Poisson with current M
            phi_matrix = 0;
            phi_rhs = 0;
            ...
            assemble_poisson_system<dim>(
                phi_dof, M_dof,
                Mx_owned_vec, My_owned_vec,  // Use current M (not M_old!)
            ...
            // Step 2: Solve Magnetization with updated φ
            M_matrix = 0;
            rhs_Mx = 0;
            rhs_My = 0;
            ...
            // Save old M for convergence check
            dealii::TrilinosWrappers::MPI::Vector Mx_prev = Mx_owned_vec;
            dealii::TrilinosWrappers::MPI::Vector My_prev = My_owned_vec;
            ...
            // Check convergence
            Mx_prev -= Mx_owned_vec;
            My_prev -= My_owned_vec;
            double local_diff = Mx_prev.l2_norm() + My_prev.l2_norm();
            double global_diff;
            MPI_Allreduce(&local_diff, &global_diff, 1, MPI_DOUBLE, MPI_SUM, mpi_communicator);

            if (global_diff < picard_tol)
                break;
        }
    ...
    // NS errors
    {
        ...
        for (const auto& cell : p_dof.active_cell_iterators())
        {
            ...
            {
                const double JxW = fe_values_p.JxW(q);
                const double p_err = (p_vals[q] - p_mean) - (exact_p.value(fe_values_p.quadrature_point(q)) - exact_p_mean);
