// ============================================================================
// utilities/mpi_tools.h - MPI Utility Functions
//
...
/**
 * @brief Broadcast a string from root to all ranks
 */
inline void broadcast(std::string& s, int root, MPI_Comm comm = MPI_COMM_WORLD)
{
    int len = 0;
    int my_rank = rank(comm);
    ...
    // MPI_Bcast requires non-const char*
    MPI_Bcast(const_cast<char*>(s.data()), len, MPI_CHAR, root, comm);
}
