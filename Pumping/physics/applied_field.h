...
// ============================================================================
// Compute ∇h_a (Jacobian of applied field)
//
// For dipole sources:
//   ∂(h_a_i)/∂p_j = α/R⁴ [-2 d_i r_j - 2 d_j r_i
//                          - 2(d·r)δ_{ij} + 8(d·r)r_i r_j/R²]
//
// where differentiation is w.r.t. evaluation point p (∂r_k/∂p_j = -δ_{kj}).
//
// NOTE: This function is currently unused. H = ∇φ is the total field
// (h_a is encoded into φ via Poisson RHS), so grad(H) = hess(φ) suffices.
// This would only be needed if h_a were added separately to H.
// ============================================================================
template <int dim>
inline dealii::Tensor<2, dim> compute_applied_field_gradient(
    ...
    double current_time)
{
    dealii::Tensor<2, dim> grad_h_a;

    // Uniform field: gradient is zero
    if (params.uniform_field.enabled)
    {
