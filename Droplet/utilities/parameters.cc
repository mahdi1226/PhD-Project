...
void Parameters::setup_droplet_magnetic()
{
    // Domain: unit square (Afkhami 2010 uses normalized coordinates)
    domain.x_min = 0.0;
    domain.x_max = 1.0;
    domain.y_min = 0.0;
    domain.y_max = 1.0;
    ...
    // Initial condition: circular droplet at center
    // Afkhami 2010: initially spherical droplet
    ic.type = 2;
    ic.droplet_center_x = 0.5;
    ic.droplet_center_y = 0.5;
    ic.droplet_radius = 0.2;  // Smaller radius to fit deformation

    // No dipoles - using uniform field instead
    dipoles.positions.clear();
    dipoles.direction = {0.0, 1.0};
    dipoles.intensity_max = 0.0;
    dipoles.ramp_time = 0.0;

    // Uniform vertical field for Afkhami 2010 comparison
    // Bo_m = μ₀ χ₀ H_a² R / σ
    // For Bo_m = 1.0, χ₀ = 0.5, R = 0.2, σ ~ λ = 0.05:
    // H_a = sqrt(Bo_m * σ / (μ₀ χ₀ R)) ≈ 1000 A/m (order of magnitude)
    uniform_field.enabled = true;
    uniform_field.magnitude = 5000.0;    // Adjust for desired Bo_m
    uniform_field.direction = {0.0, 1.0}; // Vertical field
    uniform_field.ramp_time = 0.1;        // Quick ramp to avoid shock

    // Time-stepping
    time.dt = 1e-4;           // Smaller dt for magnetic dynamics
    time.t_final = 2.0;
    time.max_steps = 20000;
    time.use_adaptive_dt = false;

    // Mesh - finer for magnetic case
    mesh.initial_refinement = 6;
    mesh.use_amr = true;
    mesh.amr_interval = 10;
    mesh.amr_max_level = 8;

    // Enable all subsystems
    enable_magnetic = true;
    enable_ns = true;
    enable_gravity = false;   // No gravity for pure magnetic deformation
...
Parameters Parameters::parse_command_line(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        else if (std::strcmp(argv[i], "--droplet_magnetic") == 0)
            params.setup_droplet_magnetic();
        ...
        else if (std::strcmp(argv[i], "--field") == 0)
        {
            if (++i >= argc) { std::cerr << "--field requires a value\n"; std::exit(1); }
            params.uniform_field.magnitude = std::stod(argv[i]);
        ...
        else if (std::strcmp(argv[i], "--langevin") == 0)
            params.use_langevin = true;
        ...
        else if (std::strcmp(argv[i], "--help") == 0 || std::strcmp(argv[i], "-h") == 0)
        {
            ...
            std::cout << "  --droplet_magnetic  Droplet with uniform field (Afkhami 2010)\n\n";
            ...
            std::cout << "  --field H       Uniform field magnitude [A/m]\n\n";
            ...
            std::cout << "  --langevin            Use Langevin nonlinear magnetization\n\n";
    ...
    if (params.output.verbose)
    {
        ...
        std::cout << "  Langevin: " << (params.use_langevin ? "ON" : "OFF") << "\n";
        if (params.uniform_field.enabled)
            std::cout << "  Uniform field: " << params.uniform_field.magnitude << " A/m\n";
        std::cout << "  Subsystems: "
