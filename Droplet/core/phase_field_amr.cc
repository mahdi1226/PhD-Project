...
void PhaseFieldProblem<dim>::refine_mesh()
{
    ...
    // Both refinement and coarsening are based on θ (phase field) for consistency
    ...
    if (params_.enable_ns)
    {
        ...
        direct_solve_countdown_ = std::max(direct_solve_countdown_, 3);  //Only 3 direct solves
