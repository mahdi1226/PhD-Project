...
void NSAssembler<dim>::assemble(double dt, double current_time)
{
    ...
    for (const auto& cell : problem_.ux_dof_handler_.active_cell_iterators())
    {
        ...
        for (unsigned int q = 0; q < n_q_points; ++q)
        {
            ...
            kelvin_force[0] =0; // mu_0 * (mx_vals[q] * h[0] + my_vals[q] * h[1]);
            if constexpr (dim >= 2)
                kelvin_force[1] = 0; //mu_0 * (mx_vals[q] * h[0] + my_vals[q] * h[1]);

            // Capillary force: (λ/ε) θ ∇ψ
            dealii::Tensor<1, dim> capillary_force;
            capillary_force = (lambda / epsilon) * theta_vals[q] * grad_psi_vals[q];

            // DIAGNOSTIC - print max values once per assembly
            static double max_cap = 0;
            if (capillary_force.norm() > max_cap)
            {
                max_cap = capillary_force.norm();
                std::cout << "New max capillary: " << max_cap
                          << " theta=" << theta_vals[q]
                          << " |grad_psi|=" << grad_psi_vals[q].norm() << std::endl;

                // After computing capillary_force:
                if (capillary_force.norm() > 1e6)
                    std::cout << "HUGE capillary at q=" << q << ": " << capillary_force.norm()
                        << " theta=" << theta_vals[q] << " |grad_psi|=" << grad_psi_vals[q].norm() << std::endl;
            }
